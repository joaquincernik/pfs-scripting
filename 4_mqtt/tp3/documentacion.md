...
Dispara la toma de una foto en el contenedor `snapshot_service` (TCP `127.0.0.1:5000`). El
contenedor ejecuta ffmpeg y publica la imagen en base64 por MQTT en el tópico `camara/snapshot`.
El comando **solo dispara** y confirma; no devuelve la foto.

- `content` (éxito): `"foto tomada y publicada en camara/snapshot"`.
- `content` (error): mensaje del contenedor o motivo de la falla de conexión.

Host y puerto del contenedor configurables con las variables de entorno `SNAPSHOT_HOST`
(defecto `127.0.0.1`) y `SNAPSHOT_PORT` (defecto `5000`).
...
```json
>> SERVER: {"err":false,"command":"snapshot","content":"foto tomada y publicada en camara/snapshot"}
```
...
| `comandos.mjs` | Implementación de cada comando (`getOsInfo`, `watch`, `getWatches`, `ps`, `oscmd`, `snapshot`). |
